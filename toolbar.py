...
REAL_MODIFIERS = {
    'MIRROR', 'SUBSURF', 'MULTIRES', 'REMESH', 'BOOLEAN', 'BEVEL',
    'SOLIDIFY', 'ARRAY', 'CURVE', 'HOOK', 'SCREW', 'LATTICE',
    'SHRINKWRAP', 'SKIN', 'DISPLACE', 'SIMPLE_DEFORM', 'ARMATURE',
    'DECIMATE', 'DATA_TRANSFER', 'NODES', 'CAST', 'CORRECTIVE_SMOOTH',
    'BUILD', 'CLOTH', 'COLLISION', 'EXPLODE', 'FLUID',
    'LAPLACIANDEFORM', 'LAPLACIANSMOOTH', 'MASK', 'MESH_CACHE',
    'MESH_DEFORM', 'MESH_SEQUENCE_CACHE', 'NORMAL_EDIT',
    'PARTICLE_INSTANCE', 'PARTICLE_SYSTEM', 'SMOOTH',
    'SURFACE_DEFORM', 'SOFT_BODY', 'TRIANGULATE', 'UV_WARP',
    'VERTEX_WEIGHT_EDIT', 'VERTEX_WEIGHT_MIX',
    'VERTEX_WEIGHT_PROXIMITY', 'WARP', 'WAVE', 'WELD',
    'WIREFRAME', 'WEIGHTED_NORMAL',
}
...
class VIEW3D_MT_touchscreen_favorites(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        #
        # Do NOT add custom operators here.
        # This keeps Add to Quick Favorites / Remove from Quick Favorites
        # completely controlled by Blender.
...
def _active_view3d_tool_id(context):
    ...
    try:
        tools = context.workspace.tools.from_space_view3d_mode(
            context.mode
        )
...
def _repeat_last_supported(context):
    ...
    if context.mode == 'EDIT_MESH':
        return (
            _active_view3d_tool_id(context)
            in EDIT_REPEAT_LAST_TOOLS
        )
